...
console.log(profile);
				...
				return Promise.using(resources.db.disposer(), function(conn) {
					...
					return users.query(conn, {email: profile.nameID})
					.then(function(u){

						// no such user, create a new onw
						if(!u.length) return users.create(conn, {
							email: profile.nameID,
							name: profile['urn:oid:1.3.6.1.4.1.5923.1.1.2'] || '',
				...
				.then(function(user) {
					...
					// redirect the user
					var query = qs.stringify(Object.assign({}, req.query, {token: token}));
					res.redirect(req.protocol + '://' + (req.headers.host || req.hostname) + config.root + '/#/?' + query);
				})
