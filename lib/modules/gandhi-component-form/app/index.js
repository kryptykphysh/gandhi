	...
	return {
		...
		controller: ['$scope', '$element', '$attrs', '$window', 'Stage', function($scope, $element, $attrs, $window, Stage) {
			...
			$scope.complete = function(form){
				...
				// persist the form ID
				$scope.content.data.formId = $scope.content.data.formId || $scope.stage.component.options.defaultFormId || 'original';
	...
	return {
		...
		controller: ['$scope', '$element', '$attrs', 'Stage', function($scope, $element, $attrs, Stage) {
			...
			$scope.toggleStageFormEdit = function(){
				$scope.ui = {newFormId: ''};
				$scope.stageFormEdit = $scope.stageFormEdit ? null : new Stage({component:{options:{
					forms: $scope.stage.component.options.forms || (Array.isArray($scope.stage.component.options.form) ? {original: $scope.stage.component.options.form} : null) || {original: ['*']},
					defaultFormId: $scope.stage.component.options.defaultFormId || 'original',
				}}});
			};
			...
			$scope.addForm = function(){
				if ($scope.stageFormEdit.component.options.forms[$scope.ui.newFormId]) {
					return alert('There is already a form with the ID "' + $scope.ui.newFormId + '"!');
				}

				$scope.stageFormEdit.component.options.forms[$scope.ui.newFormId] = ['*'];
				$scope.ui.newFormId = '';
			};
			$scope.removeForm = function(formId){
				if (formId === $scope.stageFormEdit.component.options.defaultFormId) {
					return alert('You cannot delete the default form.');
				}

				if (!confirm('Do NOT remove a form which has already been used, unless you know what you are doing! Are you sure you want to proceed?')) {
					return;
				}

				delete $scope.stageFormEdit.component.options.forms[formId];
			};
