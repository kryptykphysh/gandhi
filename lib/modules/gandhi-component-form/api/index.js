	...
	resources.components.form = {
		stage: {
			...
			write: function(conn, input, stage) {
				var result = _.merge({}, stage, input, function(sourceValue, objectValue) {
				...
				if(result.component && result.component.permissions) {
					...
					// write
					result.component.permissions.write = ( (input.component && input.component.permissions && input.component.permissions.write) ?
						input.component.permissions.write :
						result.component.permissions.write ) || {};
				}
		...
		content: {
			...
			write: function(conn, input, content, stage) {
				...
				var form = _.merge({}, content.data.form || {}, input.data ? input.data.form || {} : {}, function(sourceValue, objectValue) {
					if (Array.isArray(objectValue)) return objectValue;
				});
				...
				// TODO: this probably isn't right, but was an explicit request by Sarah...
				: null; // validator.validate(stage.component.options.schema, form, { removeAdditional: true, checkRequired: false, useDefault: false });
				...
				return {
					...
					data: {
						instructions: null,
						formId: input.data.formId || stage.component.options.defaultFormId || 'original',
						form: form
					},
					exports: form
				};
