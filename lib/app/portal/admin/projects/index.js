					...
					stages.forEach(function(s){
						...
						_.each(s.component.manifest, function(e) {
							stageColumns.push({
								...
								group: 'contents',
								content: s.id,
								path: ['contents', s.id, 'exports', e.id],
								sortable: false,
								include: 'contents',
								template: e.template
							});
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'ProjectInvitation', 'User', 'Role', function($scope, $state, $stateParams, $window, ProjectInvitation, User, Role) {
			...
			$scope.toggleCreate = function() {
				...
				if ($scope.invitationCreate) {
					$scope.invitationCreate.message = '' +
						'<p>Dear {{name}},</p>' +
						'<p>You have been invited to collaborate on the Neubauer Collegium project proposal, "{{title}}".</p>' +
						'<p>Please use the following link and token to accept your invitation: <a href="{{link}}">link here</a>.  (You will need to submit by clicking “Go!” after you follow this link.)</p>' +
						'<hr>' +
